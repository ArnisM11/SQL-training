...
const insertMovieGenres = (
  ...
  const filteredGenres = genreRows.filter(x=>genres.includes(x.genre))
  return (
    `insert into movie_genres (movie_id, genre_id) values` +
    filteredGenres.map(x => `('${movieId}','${x.id}')`).join(",")
  );
};
...
const insertMovieActors = (
  ...
  const filteredActors = actorRows.filter(x=>actors.includes(x.full_name))
  return (
    `insert into movie_actors (movie_id, actor_id) values` +
    filteredActors.map(x => `('${movieId}','${x.id}')`).join(",")
  );
};
...
const insertMovieDirectors = (
  ...
  const filtered = directorRows.filter(x=>directors.includes(x.full_name))
  return (
...
const insertMovieKeywords = (
  ...
  const filtered = keywordRows.filter(x=>keywords.includes(x.keyword))
  return (
...
const insertMovieProductionCompanies = (
  ...
  const filtered = productionCompanyRows.filter(x=>productionCompanies.includes(x.company_name))
  return (
