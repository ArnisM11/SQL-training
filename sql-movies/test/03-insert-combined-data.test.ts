...
const insertRatings = (movieId: number, ratings: Rating[]) => {
  const values = ratings.map(rating => `(${rating.userId}, ${rating.rating}, ${movieId} ,"${rating.time_created}")`).join(",");
  return `insert into movie_ratings (user_id, rating, movie_id, time_created) values ${values};`;
};
