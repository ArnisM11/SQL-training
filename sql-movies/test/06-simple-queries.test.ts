...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT(*) as count, CAST(strftime('%Y',release_date) AS TEXT) as year
      FROM movies
      GROUP BY year
      ORDER BY year DESC`;
      const result = await db.selectMultipleRows(query);
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT(*) as count, CAST(strftime('%m',time_created) AS TEXT) as month
        FROM movie_ratings
        GROUP BY month
        ORDER BY count DESC
        `;
