...
const insertProductionCompanies = (companies: string[]) => {
  return (
    ...
    companies.map(prodComp => `('${escape(prodComp)}')`).join(",")
    );
};

const insertMovies = (movies: Movie[]) => {
  return (
    `insert into movies (original_title, imdb_id, popularity, budget, budget_adjusted, revenue, revenue_adjusted, overview, runtime, release_date, homepage) values` +
    movies.map(movie => `('${escape(movie.originalTitle)}','${escape(movie.imdbId)}','${movie.popularity}','${movie.budget}','${movie.budgetAdjusted}','${movie.revenue}','${movie.revenueAdjusted}','${escape(movie.overview)}','${movie.runtime}','${escape(movie.releaseDate)}', '${escape(movie.homepage)}')`).join(",")
    );
};
